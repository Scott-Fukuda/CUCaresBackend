...
def marked_as_attended():
    ...
    try:
        ...
        user_opportunity = UserOpportunity(
            user_id=user_id,
            opportunity_id=opportunity_id,
            registered=False, # Keep marked as not registered
            attended=True  # Match your model field spelling
        )
        db.session.add(user_opportunity)
        db.session.commit()
        return jsonify({"message": "Marked as attended and registered"}), 201
...
@app.route('/api/orgs/api/<int:org_id>', methods=['GET'])
...
@app.route('/api/orgs/api/<int:org_id>', methods=['PUT'])
def update_organization(org_id):
    ...
    try:
        ...
        # Only update fields that exist in the model
        valid_fields = ['name', 'description', 'member_count', 'points', 'host_user_id']
        for field in valid_fields:
            if field in data:
...
@app.route('/api/orgs/api/<int:org_id>', methods=['DELETE'])
...
def create_opportunity():
    ...
    try:
        ...
        # Validate required fields
        required_fields = ['name', 'host_org_id', 'host_user_id', 'date']
        if not all(field in data for field in required_fields):
            return jsonify({
                'message': 'Missing required fields',
                'required': required_fields
        ...
        existing = UserOpportunity.query.filter_by(user_id=data['host_user_id'], opportunity_id=new_opportunity.id).first()
...
@app.route('/api/opps/api/<int:opp_id>', methods=['GET'])
...
@app.route('/api/opps/api/<int:opp_id>', methods=['PUT'])
def update_opportunity(opp_id):
    ...
    try:
        ...
        for field in valid_fields:
            if field in data:
                if(field == 'date'):
                    new_value = datetime.strptime(data['date'], '%Y-%m-%dT%H:%M:%S') # converts to datetime object
                elif(field == 'host_org_id'): # if host org changes, update this in other models
                    new_value = data['host_org_id']
                    # remove opportunity from previous org
                    existing = Organization.query.filter_by(id = opp.organization_id, opportunity_id=opp_id).first()
                    ex

                    # add opportunity to new org
                    existing
                    db.session.commit()
                elif(field == 'host_user_id'): # if host user changes, update this in UserOpportunity model
                    new_value = data['host_user_id']

                    # remove opportunity from previous user
                    existing = UserOpportunity.query.filter_by(user_id=opp.user_id, opportunity_id=opp.id).first()
...
@app.route('/api/opps/api/<int:opp_id>', methods=['DELETE'])
