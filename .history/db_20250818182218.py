...
class Opportunity(db.Model):
    ...
    def serialize(self):
        ...
        if self.date:
            ...
            # If date is timezone-aware, convert it; otherwise assume it's UTC
            if self.date.tzinfo is None:
                # Assume UTC if no timezone info
                date_utc = self.date.replace(tzinfo=timezone.utc)
            else:
                date_utc = self.date
            date_eastern = date_utc.astimezone(eastern_tz)
        else:
            date_eastern = None
