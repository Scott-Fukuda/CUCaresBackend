...
OPPORTUNITY_UPLOAD_FOLDER = 'static/uploads/opportunities'
...
def save_opportunity_image(file, opportunity_id):
    """Save opportunity image with opportunity ID-based filename"""
    if file and allowed_file(file.filename):
        # Get file extension
        file_extension = file.filename.rsplit('.', 1)[1].lower()
        
        # Create filename: imageUrl_{opportunity_id}.extension
        filename = f"imageUrl_{opportunity_id}.{file_extension}"
        
        # Ensure upload directory exists
        os.makedirs(OPPORTUNITY_UPLOAD_FOLDER, exist_ok=True)
        
        # Save file
        file_path = os.path.join(OPPORTUNITY_UPLOAD_FOLDER, filename)
        file.save(file_path)
        
        # Return the relative path for database storage
        return f"/static/uploads/opportunities/{filename}"
...
def create_opportunity():
    ...
    try:
        ...
        new_opportunity = Opportunity(
            name=data['name'],
            description=data.get('description'),
            date=parse_date_string(data['date']), # converts to datetime object
        ...
        if image_file:
            image_url = save_opportunity_image(image_file, new_opportunity.id)
            if image_url:
                new_opportunity.image_url = image_url
                db.session.commit()
...
def update_opportunity(opp_id):
    ...
    try:
        ...
        # Only update fields that exist in the model
        valid_fields = ['name', 'description', 'date', 'completed', 'nonprofit', 'total_slots',
                       'host_org_id', 'host_user_id']       
        
        for field in valid_fields:
            if field in data:
                if(field == 'date'):
                    new_date = parse_date_string(data['date']) # converts to datetime object
        ...
        # Handle image upload if provided
        if image_file:
            image_url = save_opportunity_image(image_file, opp.id)
            if image_url:
...
def uploaded_profile_file(filename):
    """Serve uploaded profile files"""
    return send_from_directory(PROFILE_UPLOAD_FOLDER, filename)

@app.route('/static/uploads/opportunities/<filename>')
def uploaded_opportunity_file(filename):
    """Serve uploaded opportunity files"""
    return send_from_directory(OPPORTUNITY_UPLOAD_FOLDER, filename)
