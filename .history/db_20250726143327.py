...
class UserOpportunity(db.Model):
    ...
    # Relationships back to User and Opportunity
    user = db.relationship("User", back_populates="opportuntites_involved")
    opportunity = db.relationship("Opportunity", back_populates="involved_users")
...
class User(db.Model):
    ...
    organizations = db.relationship("Organization", secondary=user_organization, back_populates="users", cascade="all")  
    opportunities_hosted = db.relationship("Opportunity", back_populates="host_user")  
    opportunities_involved = db.relationship(
        "UserOpportunity", back_populates="user", cascade="all, delete-orphan"
    )
    ...
    def serialize(self):
        return {
            ...
            "opportunities_hosted": [
                ...
                for l in self.opportunities_hosted], 
            "opportunities_involved": [  # registered/attended info
                {
                    "name": uo.opportunity.serialize(),
                    "registered": uo.registered,
                    "attended": uo.attended,
                } for uo in self.opportunities_involved
            ]
        }
...
class Organization(db.Model):
    ...
    host_user = db.relationship("User", backref="hosted_organizations")
    ...
    def serialize(self):
        return {
            ...
            "opportunities_attended": [{ opp.name() } for opp in self.opportunities_attended]
        }
...
class Opportunity(db.Model):
    ...
    involved_users = db.relationship(
        "UserOpportunity", back_populates="opportunity", cascade="all, delete-orphan"
    )
    ...
    def serialize(self):
        return {
            ...
            "involved_users": [
                {
                    "user": uo.user.serialize(),
                    "registered": uo.registered,
                    "attended": uo.attended,
                }
                for uo in self.involved_users
            ],
            "participating_organizations": [
                { org.name }
             for org in self.participating_organizations]
        }
