...
class User(db.Model):
    ...
    organizations = db.relationship("Organization", cascade="delete")   
    opportunities_hosted = db.relationship("Opportunity", back_populates="host_user")  
    user_opportunities = db.relationship(
    ...
    def serialize(self):
        return {
            ...
            "opportunities_hosted": [l.serialize() for l in self.opportunities_hosted], 
            "opportunities_involved": [  # registered/attended info
                {
                    "opportunity": uo.opportunity.serialize(),
...
class Organization(db.Model):
    ...
    def serialize(self):
        return {
            ...
            "opportunities_attended": [opp.serialize() for opp in self.opportunities_attended]
        }
...
class Opportunity(db.Model):
    ...
    name = db.Column(db.String, nullable=False)
    description = db.Column(db.String, nullable=True)
    date = db.Column(db.String, nullable=False)  # Consider using DateTime
    ...
    host_user_id = db.Column(db.Integer, db.ForeignKey("user.id"), unique=True)
    host_user = db.relationship("User", back_populates="opportunities_hosted")

    involved_users = user_opportunities = db.relationship(
    ...
    def serialize(self):
        return {
            ...
            "participating_organizations": [org.serialize() for org in self.participating_organizations]
        }
