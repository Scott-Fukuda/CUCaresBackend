...
from db import db, User, Organization, Opportunity
from models import UserOpportunity
...
@app.route('/api/users/api/<int:user_id>', methods=['GET'])
...
@app.route('/api/users/api/<int:user_id>', methods=['PUT'])
...
@app.route('/api/users/api/<int:user_id>', methods=['DELETE'])
...
def update_opportunity(opp_id):
    ...
    try:
        ...
        for field in valid_fields:
            if field in data:
                if(field == 'date'):
                    setattr(opp, field, datetime.strptime(data['date'], '%Y-%m-%dT%H:%M:%S')) # converts to datetime object
                else:
