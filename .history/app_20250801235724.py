...
def marked_as_attended():
    ...
    opportunity_id = data.get('opportunity_id')
    opp_dur = Opportunity.query.filter_by(id=opportunity_id).duration
    ...
    try:
        ...
        if existing:
            ...
            db.session.commit()
            return jsonify({"message": "Attendance updated & points updated"}), 200
...
def create_organization():
    ...
    try:
        ...
        // TODO throw error if host_user_id doesnt exist
...
def create_opportunity():
    ...
    try:
        ...
        # Validate required fields
        required_fields = ['name', 'host_org_id', 'host_user_id', 'date', 'cause']
        if not all(field in data for field in required_fields):
            return jsonify({
                'message': 'Missing required fields',
                'required': required_fields
        ...
        # TODO this could error if host_user does not exist
